...
const char *luaO_pushfstring (lua_State *L, const char *fmt, ...) {
  ...
  /* Prevent recursion and only color primary message strings */
  if (fmt != NULL && fmt[0] != '\x1b') {
    /* ** \x1b[94m  = Light Blue
    ** \x1b[31m  = Red
    ** \x1b[0m   = Reset
    */
    
    /* If the message starts with 'lua:', color it blue and the rest red */
    if (strncmp(msg, "lua:", 4) == 0) {
        lua_pushfstring(L, "\x1b[94mlua:\x1b[31m%s\x1b[0m", msg + 4);
    } else {
        /* Otherwise, just make the whole thing red */
        lua_pushfstring(L, "\x1b[31m%s\x1b[0m", msg);
    }
